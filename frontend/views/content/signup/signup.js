myApp.controller('SignupCtrl', function ($scope, toastr, TemplateService, NavigationService, $state) {
    ...
    $scope.validatePassword = function (value) {
        ...
        if (value.length >= 10 && value.length <= 128 && hasNumber.test(value) && hasLowercase.test(value) && hasUppercase.test(value)) {
            isValid = true;
        } else {
            return false;
        }
        ...
        for (var x = 0; x < value.length; x++) {
            if (value.charAt(x) == value.charAt(x + 1) && value.charAt(x) == value.charAt(x + 2)) {
                return false;
            }
        };
    ...
    $scope.userSignup = function (value) {
        ...
        var isValidPassword = $scope.validatePassword(value.password);

        if (isValidPassword) {
            NavigationService.userSignup("User/userSignup", value, function (data) {
                if (data.value) {
